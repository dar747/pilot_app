...
from analyze import analyze_notam
...
def save_to_db(result, raw_text, notam_number, raw_hash):
    session = SessionLocal()
    try:
        exists = session.query(NotamRecord).filter(
            (NotamRecord.airport == result.airport) &
            (NotamRecord.notam_number == notam_number)
        ).first()

        if exists:
            print(f"⏩ Skipping duplicate NOTAM {notam_number} at {result.airport}")
            return
        ...
        session.add(record)
        session.commit()
        print(f"📝 Saved {result.notam_number} at {result.airport}")
...
async def run_analysis(to_analyze: List[Dict], batch_size=200):
    ...
    for i in range(0, len(to_analyze), batch_size):
        ...
        tasks = [analyze_notam(n["icao_message"]) for n in batch]
